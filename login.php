...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["loginId"]))){
        $loginId_err = "Please enter login id.";
    } else{
    ...
    if(empty($loginId_err) && empty($password_err)){
        // Prepare a select statement
        $sql = "SELECT loginId, password FROM authentication WHERE loginId = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $loginId, $hashed_password);
                    ...
                    $loginId_err = "No account found with that username.";
                }
            } else{
...
<body>
    <div class="wrapper">
        <h2>Mahatma Gandhi Central library Login</h2>
